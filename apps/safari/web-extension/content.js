(() => {
  ...
  const text=e=>(e.getAttribute('aria-label')||e.labels?.[0]?.innerText||e.innerText||e.getAttribute('placeholder')||e.getAttribute('alt')||'').trim().slice(0,200);
  ...
  function snapshot() {
    ...
    for(const e of document.querySelectorAll('a[href],button,input,textarea,select,[role=button],[contenteditable=true]')) {
      if(!visible(e)||secretField(e)||e.matches('input[type=hidden]'))continue;
      if(count++>=120)break;
  ...
  browser.runtime.onMessage.addListener(async(message,sender)=>{
    ...
    if(message.op==='snapshot')return snapshot();
