...
if(process.argv.includes('--broker')) {
  ...
  tool('safari_execute','Execute 1–10 DOM actions on an assigned INACTIVE tab using fresh snapshot refs. DOM input is not native trusted input. Stops for challenges/user takeover. Never solve CAPTCHAs, submit payments, or enter secrets without required user authorization. If status is unknown, inspect safari_result; do not retry with a new request ID. Resume is available only in the human extension popup.',{tabHandle:z.string(),requestId:z.string().min(1).max(100),steps:z.array(step).min(1).max(10)},a=>client.tab(a.tabHandle).execute(a.steps,a.requestId));
