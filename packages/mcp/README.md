...
`main.mjs` uses the official MCP SDK and stdio transport. Nine tools cover status, pairing, domain-filtered tab discovery, background opening, snapshot, bounded DOM actions, same-origin navigation, request results and revocation. Resume remains a human-only extension popup action.
