...
if(process.argv.includes('--broker')) {
  ...
  tool('safari_tabs','List Safari tab titles and URLs for the user-requested domain only, including its subdomains. Does not select or modify tabs.',{domain:z.string()},a=>client.findTabs(a.domain),true);
  tool('safari_open','Open a new INACTIVE Safari tab for a user-requested URL and return its task handle. Reuses existing Safari website permissions and login session. Never activates a tab or asks for browser permissions in the background.',{url:z.string().url(),requestId:z.string().min(1).max(100)},a=>client.open(a.url,a.requestId));
  tool('safari_snapshot','Read a fresh bounded text/element snapshot of an assigned tab. References expire on navigation or another snapshot. Password values are excluded.',{tabHandle:z.string(),match:z.string().max(100).optional()},a=>client.tab(a.tabHandle).snapshot(a.match),true);
